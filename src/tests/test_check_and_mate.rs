...
fn test_is_king_in_check() {
    ...
    assert!(!board.is_king_in_check(Color::White).unwrap_or(true), "White king is not in check by white queen");
    ...
    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black queen");
    ...
    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black rook");
    

    board.grid[4][3] = Some(Piece::Pawn(Color::Black)); // Black pawn at d5
    board.pieces.insert(Piece::Pawn(Color::White), HashSet::from([(5, 4)]));

    assert!(!board.is_king_in_check(Color::White).unwrap_or(true), "Check by rook is blocked by pawn");

    board.grid[4][4] = Some(Piece::Pawn(Color::Black)); // Black pawn at e5
    board.pieces.insert(Piece::Pawn(Color::Black), HashSet::from([(4, 4)]));

    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black pawn");
    ...
    assert!(!board.is_king_in_check(Color::White).unwrap_or(true), "Check by black rook is blocked by black pawn, white pawn doesn't check");

    board.grid[5][5] = Some(Piece::King(Color::Black)); // Black king at f6
    board.pieces.insert(Piece::King(Color::Black), HashSet::from([(5, 5)]));

    assert!(board.is_king_in_check(Color::Black).unwrap_or(false), "Black king is in check by white pawn");
    ...
    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black bishop");
    ...
    assert!(!board.is_king_in_check(Color::White).unwrap_or(true), "Check by black bishop is blocked by white bishop");
    ...
    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black knight");

    board.grid = [[None; 8]; 8];
    board.pieces = Default::default(); // Empty hashmap

    assert!(board.is_king_in_check(Color::White).is_err(), "King not found");
    ...
    assert!(board.is_king_in_check(Color::White).is_err(), "Multiple kings found");
    ...
    assert!(board.is_king_in_check(Color::White).unwrap_or(false), "White king is in check by black king");
    assert!(board.is_king_in_check(Color::Black).unwrap_or(false), "Black king is in check by white king");
