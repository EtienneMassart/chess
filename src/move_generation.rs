...
impl Board {
    pub fn generate_pawn_moves(&self, start: (usize, usize), color: Color) -> Vec<(usize, usize)> {
        ...
        potential_moves.into_iter()
            .filter_map(|pos| utils::is_valid_position(pos))
            .collect()
    }

    pub fn generate_knight_moves(&self, start: (usize, usize)) -> Vec<(usize, usize)> {
        let potential_moves = vec![
    ...
    pub fn generate_bishop_moves(&self, start: (usize, usize)) -> Vec<(usize, usize)> {
        let mut moves = Vec::new();

        for direction in &[(1, 1), (1, -1), (-1, 1), (-1, -1)] {
            let mut pos = (start.0 as isize + direction.0, start.1 as isize + direction.1);
            while let Some(new_pos) = utils::is_valid_position(pos) {
                moves.push(new_pos);
    ...
    pub fn generate_rook_moves(&self, start: (usize, usize)) -> Vec<(usize, usize)> {
        let mut moves = Vec::new();
    ...
    pub fn generate_queen_moves(&self, start: (usize, usize)) -> Vec<(usize, usize)> {
        let mut moves = Vec::new();
    ...
    pub fn generate_king_moves(&self, start: (usize, usize)) -> Vec<(usize, usize)> {
        let potential_moves = vec![
    ...
    pub fn get_legal_moves(&mut self, start: (usize, usize), game_state: &GameState) -> Vec<(usize, usize)> {
    ...
    pub fn has_legal_moves(&mut self, color: Color, game_state: &GameState) -> bool {
        for i in 0..8 {
            for j in 0..8 {
